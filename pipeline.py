...
def validate_360_image(image_path):
    ...
    try:
        ...
        from scripts_demo_yolo.scripts.image_validator import ImageValidator
...
def main():
    parser = argparse.ArgumentParser(description="Pipeline de análisis de imágenes 360° con validación automática")
    ...
    parser.add_argument("--force", action="store_true",
                       help="Forzar procesamiento sin validación 360°")
    ...
    for i, image_path in enumerate(image_files, 1):
        ...
        # Validar imagen 360° (solo si no se fuerza)
        if not args.force:
            if not validate_360_image(image_path):
                print(f"⚠️ Imagen saltada - No es 360° equirectangular")
                skipped += 1
                continue
        ...
        try:
            ...
            print(f"🚀 Iniciando pipeline para imagen: {image_path}")
            ...
            print(f"\n✅ Imagen procesada exitosamente: {image_name}")
            processed += 1
    ...
    print(f"Imágenes saltadas (no 360°): {skipped}")
